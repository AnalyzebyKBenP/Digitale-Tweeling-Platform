from flask import Blueprint, current_app, Response, request, abort, render_template, make_response
...
def generatesas(staccname,k):
    ...
    s=generate_account_sas(account_name=staccname,account_key=k,resource_types=rt,permission=p,expiry=e,protocol='https')
    sas[staccname]=s
    return s
...
def getsas(staccname,sub_id):
    # Testing parameters
    # staccname='analyzestmow001'
    # sub_id="0487c8fe-3201-4b95-ab8d-3640906a1a59"
    try:
        s=sas[staccname]
        # get expiry from sas
        result = [i for i in s.split('&') if i.startswith('se=')]
        exptime=datetime.strptime(result[0][3:].replace('%3A',':'), '%Y-%m-%dT%H:%M:%SZ')
        ...
        try:
            ...
            for b in list(a):
                if b.name==staccname.lower():
                    ...
                    storage_keys  =  storage_client.storage_accounts.list_keys(rg,staccname)
