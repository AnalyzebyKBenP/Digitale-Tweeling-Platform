...
function getThemeIcon(theme) {
    themeIcons = {
        ...
        "Luchtfoto": "fa-camera",
        "Maatschappelijk": "fa-handshake",
        "Milieusensoren": "fa-temperature-high",
        "Mobiliteit": "fa-car-side",
        "Natuur": "fa-leaf",
        "Openbare ruimte (BOR)": "fa-compress",
                ...
                if (item.MenuItem) {
                    ...
                    <div class="label-container relative">
                        <label class="checkbox-button checkbox-container flex items-start relative ${item.type}" onclick="errorDataId('${item.layerName}_btn_id')">
                            ...
                            <span class="checkbox-label text-16 ml-7 font-light cursor-pointer max-w-[285px]" id="${item.layerName}_txt_id">${item.layerDisplayName}</span>
                            ${global}
                        </label>
                        <span class="layer-sliders hidden absolute text-16 -right-6 top-2 cursor-pointer [&.active]:text-primary hover:text-primary" onclick="layerSliderPopup(this,'${item.layerName}','${item.layerDisplayName}')">
...
function prepareConstructorLayer(
    ...
    t='func_as_string'
        ...
        // Check if the response of the function has to be a string or a function
        if (t == 'func_as_string'){
            return `show_layer(\\'${item.layerName}_id\\'), legend_content(\\'${legendId}\\'),openTab(this), layerClick(\\'${item.layerDisplayName}\\',\\'${item.layerName}\\')`
        }
        // Return as a function which is executed directly
        else{
            return show_layer(`${item.layerName}_id`), legend_content(`${legendId}`),openTab(this), layerClick(`${item.layerDisplayName},${item.layerName}`)
...
function layerClick(layerDisplayName, layerName) {
    ...
    if (activeLayers.querySelector('ul').childElementCount > 0) {
        ...
        document.getElementById('datalayers-panel').classList.add('layers-active');
    } else {
        activeLayers.classList.remove('!translate-y-0', '!max-h-[165px]');
        document.getElementById('datalayers-panel').classList.remove('layers-active');
    }
}
...
function tagClick(tag) {
    if (document.getElementById('active-layers').dataset.loading === 'false') {
        if (document.getElementById(tag.dataset.tag + '_btn_id').checked === true) {
...
function openTab(checkbox) {
    ...
    if (checkbox.checked && document.querySelector(`[data-btn="${checkbox.id}"]`)) {
        const tabs = document.querySelectorAll(`[data-btn="${checkbox.id}"]`);
    ...
    if (checkbox.checked && document.querySelector(`[data-btn2="${checkbox.id}"]`)) {
        const tabs = document.querySelectorAll(`[data-btn2="${checkbox.id}"]`)
        tabs.forEach(function (tab) {
            if (!tab.classList.contains('active')) {
                tab.classList.remove('!hidden')
                tab.click();
    ...
    if(checkbox.checked && document.querySelector(`[data-btn3="${checkbox.id}"]`)) {
        const tabs = document.querySelectorAll(`[data-btn3="${checkbox.id}"]`);
        tabs.forEach(function(tab) {
            if(!tab.classList.contains('active')) {
                tab.classList.remove('!hidden')
                tab.click();
    ...
    if(checkbox.checked && document.querySelector(`[data-btn4="${checkbox.id}"]`)) {
        const tabs = document.querySelectorAll(`[data-btn4="${checkbox.id}"]`);
        tabs.forEach(function(tab) {
            if(!tab.classList.contains('active')) {
                tab.classList.remove('!hidden')
                tab.click();
