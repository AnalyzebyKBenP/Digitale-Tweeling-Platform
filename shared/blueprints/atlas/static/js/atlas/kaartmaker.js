    ...
    for (const theme in jsonData) {
        if (jsonData.hasOwnProperty(theme)) 
        {
            ...
            for (const item of jsonData[theme].items) {
                if(item.details.layerType=='none') {
                    continue
                }
                // console.log(item)
                var createLayer = ""
...
function prepareConstructorBim(
    ...
    t='func_as_string'
        ...
        if (t == 'func_as_string'){
            return `toggle_visibility_threebox_model(\\'${item.details.url_m}\\', \\'${item.layerName}_btn_id\\'), view_to_coords_maps(${item.details.lon_m}, ${item.details.lat_m}, 18, 34, 60, \\'${item.layerName}_btn_id\\'), legend_content(\\'leg_${item.layerName}\\')`
        }
...
function prepareConstructorLayer(
    ...
    t='func_as_string'
        ...
        // Check if the response of the function has to be a string or a function
        if (t == 'func_as_string'){
            return `show_layer(\\'${item.layerName}_id\\'), legend_content(\\'${legendId}\\')`
        }
        // Return as a function which is executed directly
        else{
            return show_layer(`${item.layerName}_id`), legend_content(`${legendId}`)
