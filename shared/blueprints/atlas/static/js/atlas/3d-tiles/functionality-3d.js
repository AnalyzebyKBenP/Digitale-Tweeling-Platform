...
function change_3D_map(type, tileSet, pitchAngle = 0) {
    display_cesiumMap = document.getElementById('cesiumMap').style.display
    switch (type) {
        case '2D':
            if (display_cesiumMap == 'block') { show_3d_view() }
        ...
        case '2DGebouwen':
            ...
            break;
        case 'cesium':
            if (display_cesiumMap == 'none') {
                show_3d_view()
            } //Switch to 3D
            changeCesiumYear(tileSet)
            break;
        case 'pitch':
            map.map.flyTo({ pitch: pitchAngle });
            break;
